 ...
 *
 * Está escrito largo a propósito. Quien lo abre entre dos pedidos busca una
 * respuesta concreta —por qué no le deja cobrar, qué diferencia hay entre
 * recibir y contar, si cancelar o borrar— y esa respuesta tiene que estar
 * completa aquí dentro, no a medias: un manual que se queda corto obliga a
 * preguntarle al dueño, que es justamente lo que este módulo evita. Para que
 * lo largo no estorbe, el manual trae índice, buscador y secciones plegables:
 * se lee el pedazo que hace falta, no todo.
 *
 * El contenido se arma con los datos reales del negocio (zona horaria, moneda,
 * categorías del menú, módulos encendidos) en lugar de describir un sistema
 * genérico: lo que el manual explica es exactamente lo que la persona tiene
 * enfrente.
 * ========================================================================== */
...
interface ManualContext {
  ...
  rewardCost: number;
  /** Categorías activas del menú, como se ven en el filtro de la caja. */
  categories: string[];
...
/** Enumera en español: «Matcha, Café y Bakery». */
...
function buildSections(ctx: ManualContext): Section[] {
  // La lealtad tiene dos interruptores distintos: la bandera del negocio
  // (¿otorga puntos?) y la visibilidad de la interfaz (¿existe la pantalla?).
  // El manual sólo la explica cuando las dos cosas están de acuerdo, porque
  // describir una pantalla que no está confunde más que callar.
  const lealtadVisible = SHOW_LEALTAD_UI;
  const lealtadActiva = SHOW_LEALTAD_UI && ctx.lealtadOn;
  const punto = `${ctx.pointsPerCurrency} punto${ctx.pointsPerCurrency === 1 ? "" : "s"}`;
  const categorias = ctx.categories.length
    ? enumerate(ctx.categories.slice(0, 5))
    : "las que hayas creado en Productos";
  ...
  const sections: Section[] = [
    {
      ...
      blocks: [
        {
          kind: "parrafo",
          text: `Esta es la aplicación con la que se opera ${ctx.branchName}: se cobra, se preparan los pedidos, se controla el inventario${lealtadVisible ? ", se lleva el programa de lealtad" : ""} y se cierra la caja. No es una demostración ni una práctica: todo lo que registres aquí es real, se guarda en la base de datos del negocio y lo ven de inmediato las demás pantallas y los reportes.`,
        },
        {
          kind: "parrafo",
          text: "Todo el equipo trabaja sobre la misma información. Si dos cajas venden al mismo tiempo, ambas ven las mismas comandas y el mismo inventario, porque nada se guarda en el dispositivo: cada acción viaja al servidor y regresa con el estado actualizado. Por eso no existe un «modo sin conexión»; si el internet se cae, el cobro espera.",
        },
        {
          kind: "pasos",
          ...
          items: [
            ...
            "Si algo se preparó de más o se tiró, avísale a un administrador para que lo registre como merma en Inventario.",
        ...
        },
        {
          ...
          items: [
            "Inicio — el resumen del día: ventas, pedidos abiertos, insumos en alerta y lotes por vencer. Y este manual.",
            ...
            "Productos — el menú: categorías, precios, fotos, recetas y qué se puede personalizar.",
            ...(lealtadVisible
              ? ["Clientes — el programa de lealtad: puntos, visitas y tarjeta digital."]
              : []),
        ...
        },
        {
          kind: "aviso",
          tone: "amber",
          text: `Cada venta descuenta insumos de verdad${lealtadActiva ? " y suma puntos de verdad" : ""}. Si quieres practicar, avísale a un administrador: él puede borrar los tickets de prueba desde Administración de pedidos.`,
        },
      ],
    },
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "Iniciar sesión dice quién eres; lo que puedes hacer lo decide la lista del equipo. Son dos cosas distintas a propósito.",
...

    {
      ...
      audience: "todos",
      summary:
        "Menú, avisos, números en verde, el indicador de «Guardando» y cada cuánto se actualiza la información.",
      blocks: [
        {
          kind: "lista",
        ...
        },
        {
          ...
          items: [
            ...
            "En el punto de venta, el ticket se abre con el botón inferior: ahí están los totales y el botón de cobrar.",
          ],
        },
        {
          kind: "datos",
      ...
      blocks: [
        {
          ...
          items: [
            "Elige dónde se consume: «Para aquí» o «Para llevar». No es un detalle estético: decide si se descuenta el empaque.",
            `Busca el producto por nombre o filtra por categoría (${categorias}).`,
            ...
            "Aplica una promoción si corresponde: «Descuento 10 %» o «Cliente frecuente 15 %».",
            ...(lealtadActiva
              ? ["Elige al cliente de lealtad si está registrado, para que la compra le sume sus puntos."]
              : []),
        ...
        },
        {
          ...
          items: [
            ...
            ...(lealtadVisible
              ? [
                  ctx.lealtadOn
                    ? `Si elegiste cliente, se le suman ${punto} por cada peso de la compra.`
            ...
            "Todo eso pasa junto: si algo falla, no queda ni la venta ni el descuento a medias.",
          ],
        },
        {
          kind: "datos",
          ...
          items: [
            {
              ...
              desc: "El total lo calcula el servidor releyendo el precio del producto, el cargo de la leche y el de los extras. Para cambiar un precio hay que hacerlo en Productos. Así ningún cobro depende de lo que tenía cargado la tablet.",
            },
            {
              term: "La propina nunca pasa del consumo",
              desc: "Un monto mayor a la cuenta es siempre un error de dedo, así que el sistema lo topa. La propina se calcula sobre el consumo ya con descuento y se suma aparte, para que una promoción no le recorte al equipo lo que el cliente quiso dejar.",
            },
            {
              term: "Para aquí no gasta empaque",
              desc: "Si el cliente se queda, se sirve en loza y los vasos no se descuentan. Por eso vale la pena marcarlo bien: es la diferencia entre un inventario que cuadra y uno que no.",
            },
            {
              term: "La promoción aplica al consumo",
              desc: "Se descuenta de lo que se cobra por los productos. La propina se calcula después, sobre ese consumo ya rebajado.",
            },
            {
              term: "El folio es el nombre del pedido",
              desc: "Es el número consecutivo que aparece al cobrar y el que se grita en barra. Sirve para encontrar el ticket después.",
            },
          ],
        },
        {
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "Un administrador puede cancelar el ticket (devuelve insumos) o borrarlo desde la misma tarjeta.",
            "El tablero se actualiza solo: no hace falta recargar para ver lo que cobró la otra caja.",
            "Mover una tarjeta no cambia el dinero: los estados son de la barra, no de la caja.",
          ],
        },
      ],
    },

    {
      ...
      blocks: [
        ...
        {
          kind: "datos",
          title: "Los movimientos que más se confunden",
          items: [
            {
              term: "Recibir pedido",
            ...
            {
              term: "Conteo físico",
              desc: "Reemplaza el total por lo que contaste. Si cuentas 200 vasos, quedan 200 vasos exactos, sin importar lo que decía el sistema. Es la forma de volver a cuadrar.",
            },
            {
              term: "Botones + y −",
            ...
            },
            {
              ...
              desc: "Cada insumo abre la lista de productos que lo usan y con cuánto. Ahí mismo se corrige la receta o se agrega el insumo a otro producto, sin ir a Productos.",
            },
          ],
        },
        {
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "El aviso del último día no desaparece solo: sigue en rojo hasta que alguien pulsa «Ya lo revisé». Un aviso que se va solo no sirve de nada.",
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "Las leches son globales: se define su cargo extra y qué insumo descuentan, y vale para todo el menú.",
...

    {
      ...
      audience: "admin",
      summary:
        "Las categorías las creas tú: cómo se ordenan, qué pasa al ocultarlas y qué pregunta el sistema al borrar una.",
      blocks: [
        {
          kind: "parrafo",
          text: `Las categorías no vienen fijas en el sistema: se crean, se renombran, se reordenan y se borran desde Productos. Hoy el menú usa ${categorias}. Si mañana el negocio vende mercancía, matcha para llevar o temporada navideña, se agrega la categoría y ya.`,
        },
        {
          kind: "pasos",
          title: "Crear y acomodar",
          items: [
            "En Productos, abre la administración de categorías.",
            "Escribe el nombre y elige su emoji: los dos se ven en el filtro de la caja.",
            "Ordénalas como quieres que aparezcan en el punto de venta: primero lo que más se vende.",
            "Renombra cuando cambie el nombre comercial; el producto no se mueve de lugar.",
          ],
        },
        {
          kind: "datos",
          title: "Reglas de las categorías",
          items: [
            {
              term: "Ocultar no es borrar",
              desc: "Una categoría inactiva deja de ofrecerse en el filtro de la caja y en el formulario del producto, pero sus productos se siguen vendiendo con normalidad.",
            },
            {
              term: "Borrar una categoría con productos",
              desc: "El sistema no la borra a ciegas: pregunta a qué categoría se mudan esos productos. Ningún producto se queda sin categoría.",
            },
            {
              term: "Renombrar es seguro",
              desc: "El identificador interno se calcula una sola vez, al crearla, y ya no cambia. Por eso puedes renombrarla cuantas veces quieras sin tocar las ventas que ya la tienen escrita.",
            },
          ],
        },
      ],
  ...
  if (SHOW_LEALTAD_UI) {
    sections.push({
      ...
      blocks: [
        {
          kind: "parrafo",
          text: `Cada cliente registrado acumula ${punto} por cada peso de compra y tiene su tarjeta digital con un QR escaneable. El canje de una bebida cuesta ${ctx.rewardCost} puntos.`,
        },
        {
          kind: "pasos",
          title: "Cómo se usa",
  ...
  sections.push(
      ...
      blocks: [
        {
          ...
          items: [
            ...
            "Los productos más vendidos, con los que ya salieron del menú marcados como «fuera del menú»: sus ventas siguen contando, así que los totales cuadran.",
...

    {
      ...
      blocks: [
          ...
          items: [
            {
              ...
              desc: `La venta se conserva marcada como cancelada y se devuelven los insumos${lealtadActiva ? " y se retiran los puntos" : ""}. Es la herramienta del día a día: la venta ocurrió y el historial tiene que poder explicarla. Sólo se puede antes del corte de ese día.`,
            },
            {
            ...
            },
            {
              ...
              desc: `La venta desaparece de la base y de los reportes. Existe para limpiar capturas de prueba, no para operar. Antes de borrar se devuelven los insumos${lealtadActiva ? " y los puntos" : ""}, y tampoco se puede si el día ya tiene corte cerrado.`,
            },
          ],
        },
        {
        ...
        },
        {
          kind: "aviso",
          tone: "danger",
          text: "Borrar no se puede deshacer. Si dudas entre cancelar y borrar, cancela: conserva la información y devuelve los insumos igual.",
        },
      ],
    },

    {
      ...
      blocks: [
        ...
        {
          kind: "parrafo",
          text: `El día del negocio se calcula con la zona horaria configurada (${ctx.timezone}), no con la del dispositivo. Un turno que cruza la medianoche cierra donde el negocio dice que cierra, y revisar el panel desde otro huso horario no mueve el corte.`,
        },
        {
          kind: "aviso",
          tone: "amber",
      ...
      blocks: [
        {
          ...
          items: [
            ...
            `Cobrar es una sola operación. El ticket, el descuento de insumos, los movimientos${lealtadActiva ? " y los puntos" : ""} ocurren juntos: o queda todo, o no queda nada.`,
      ...
      blocks: [
        {
          ...
          items: [
            ...
            },
            {
              ...
              desc: "Haz un conteo físico para dejar la existencia real, registra las mermas con el botón −, y revisa que los productos que se venden tengan receta. Revisa también que el empaque esté marcado como empaque: si no, se descuenta también en pedidos para aquí. Si el módulo de inventario está apagado, las ventas no descuentan nada.",
            },
            {
              term: "«Un producto no aparece en la caja»",
              desc: "Está pausado, no está activo en el menú, o su categoría está oculta. Se enciende desde Productos.",
            },
            {
              term: "«Una categoría desapareció del filtro»",
              desc: "Está inactiva. Sus productos se siguen vendiendo; la categoría se vuelve a activar desde Productos.",
            },
            {
              term: "«Cobré mal un ticket»",
              desc: "Pídele a un administrador que lo cancele: devuelve los insumos. Se puede mientras no se haya cerrado el corte de ese día. Si sólo sobra un producto, se puede quitar ese renglón sin cancelar todo.",
            },
            {
      ...
      blocks: [
        {
          ...
          items: [
            ...
            { term: "Renglón", desc: "Cada producto dentro de un ticket, con su cantidad y sus modificadores." },
            ...
            { term: "Categoría", desc: "El grupo con el que se filtra el menú en la caja. La define el negocio, no el sistema." },
            ...
            { term: "Fondo de caja", desc: "El dinero con el que arranca el cajón; sirve de referencia al contar." },
          ],
        },
      ],
    },
...
export function InstructionsPanel() {
  ...
  const sections = useMemo(
    ...
    [state.settings, state.flags, state.categories],
  );
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-xl text-xs leading-5 text-muted">
              ...
              primer día, empieza por «Qué es este sistema y cómo empezar»; si
              buscas algo puntual, usa el buscador.
            </p>
          </div>
        </div>
        <Button
