-- ============================================================================
-- TomoMatcha · operaciones transaccionales
-- ----------------------------------------------------------------------------
-- Cobrar una venta toca cinco tablas a la vez (orders, order_items,
-- ingredients, inventory_movements, customers/loyalty). Se hace dentro de una
-- función para que sea atómico: o queda todo, o no queda nada.
-- Los precios SIEMPRE se leen de la base, nunca del cliente.
-- ============================================================================

-- Día operativo actual en la zona horaria del negocio.
create or replace function business_day(at timestamptz default now())
returns date
language sql
stable
set search_path = public
as $$
  select (at at time zone coalesce((select timezone from settings where id = 1), 'UTC'))::date;
$$;
...
begin
  ...
  -- No se cobra sobre un turno ya cerrado.
  ...
  insert into orders (
    ...
    cash_received, created_by, created_by_name
  ) values (
    v_folio, 0, v_discount, nullif(payload->>'discountLabel', ''), 0, v_payment, 'nuevo',
    (payload->>'cashReceived')::numeric, p_staff_id, v_staff_name
  ...
  for v_line in select * from jsonb_array_elements(payload->'lines') loop
    ...
    if v_settings.flag_inventario then
      for v_recipe in
        select ingredient_id, is_milk, qty from product_recipe_items where product_id = v_product.id
      loop
      ...
      for v_recipe in
        select eri.ingredient_id, eri.qty
...
-- ---------------------------------------------------------------------------
-- close_cash · corte del día operativo
-- ---------------------------------------------------------------------------
...
begin
  ...
  insert into cash_closes (
    ...
    orders_count, notes, closed_by, closed_by_name
  ) values (
...
-- ---------------------------------------------------------------------------
-- adjust_stock · ajuste manual con bitácora
-- ---------------------------------------------------------------------------
...
begin
  update ingredients
     set stock = greatest(0, round(stock + p_delta, 2))
  ...
  insert into inventory_movements (ingredient_id, delta, stock_after, reason, staff_id, note)
  values (p_ingredient_id, p_delta, v_stock, p_reason, p_staff_id, nullif(trim(coalesce(p_note, '')), ''));
...
-- ---------------------------------------------------------------------------
-- adjust_points · puntos manuales con bitácora
-- ---------------------------------------------------------------------------
...
begin
  ...
  insert into loyalty_transactions (customer_id, points, balance_after, reason, staff_id)
  values (p_customer_id, p_points, v_balance, coalesce(p_reason, ''), p_staff_id);
...
revoke all on function adjust_stock(uuid, numeric, movement_reason, uuid, text) from anon, authenticated;
revoke all on function adjust_points(uuid, integer, text, uuid) from anon, authenticated;
revoke all on function business_day(timestamptz) from anon, authenticated;
